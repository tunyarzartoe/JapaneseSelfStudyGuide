...
const KatakanaPage = () => {
  ...
  const filteredChars = useMemo(() => {
    if (selectedGroup === 'All') return katakana;
    const group = katakanaGroups.find(g => g.label === selectedGroup);
    return katakana.filter(c => group?.chars.includes(c.char));
  }, [selectedGroup]);
  ...
  const nextQuestion = () => {
    if (quizIndex + 1 >= quizChars.length) setShowResult(true);
    else { setQuizIndex(i => i + 1); setSelected(null); }
  };

  const restartQuiz = () => {
    setQuizIndex(0); setSelected(null); setScore(0); setTotal(0); setShowResult(false);
  };
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <h2>カタカナ Katakana</h2>
        <p className="page-subtitle">Used for foreign words, loanwords, and emphasis — master all 46+ characters</p>
      ...
      {mode === 'chart' && (
        <>
          ...
          <div className="kana-grid">
            {filteredChars.map((c) => (
              <div key={c.char} className="kana-card katakana-card" onClick={() => speak(c.char)}>
          ...
          <p className="kana-tip">💡 Click any card to hear the pronunciation! Katakana is used for words like コーヒー (coffee) and テレビ (TV).</p>
        </>
      )}
      ...
      {mode === 'quiz' && !showResult && current && (
        <div className="quiz-container">
          ...
          <div className="quiz-card">
            ...
            <p className="quiz-prompt">What is the romaji for this katakana?</p>
